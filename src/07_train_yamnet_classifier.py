"""
07_train_yamnet_classifier.py

Trains a small classifier on top of the frozen YAMNet embeddings from
Step 06. Since YAMNet already did the hard work of understanding general
audio structure (pretrained on 2M+ external clips), this classifier only
needs to learn how to map those rich embeddings to GTZAN's 10 genres --
a much easier problem than learning from raw spectrograms using only 700
songs, which is why this approach tends to reach much higher accuracy.

Uses the same song-level evaluation (segment-level + song-level via
averaged probabilities) as every other script in this project.
"""
...
MODEL_OUTPUT_PATH = "models/yamnet_classifier.keras"
...
def load_split(name):
    data = np.load(os.path.join(FEATURES_DIR, f"{name}_embeddings.npz"))
...
def build_classifier(input_dim, num_classes):
    """A small MLP head -- YAMNet already extracted the hard features,
    so we only need a few dense layers to map embeddings -> genre."""
